...
function getHtml(scriptUri, cmCssUri, themeCssUri, cmJsUri, jsModeUri, calculatorType) {
...
<head>
  ...
  <style>
    ...
    #right { flex: 3 1 0; min-width: 200px; background: transparent; position: relative; display: flex; flex-direction: column; ${hasState ? '' : 'display: none;'} }
...
function openDesmos({ viewType, script, title, restoredState, onUnsaved, calculatorType }) {
  ...
  const cmCssUri = panel.webview.asWebviewUri(vscode.Uri.file(path.join(libDir, 'codemirror.min.css'))).toString();
  const themeCssUri = panel.webview.asWebviewUri(vscode.Uri.file(path.join(libDir, 'dracula.min.css'))).toString();
  ...
  panel.webview.html = getHtml(scriptUri, cmCssUri, themeCssUri, cmJsUri, jsModeUri, calculatorType);
